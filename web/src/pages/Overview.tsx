...
export default function Overview() {
  ...
  const cols: Col[] = useMemo(() => [
    ...
    { key: 'weekend', label: '週末費/TVL', get: r => r.weekend_fee_tvl, fmt: v => v === null ? '—' : (v * 100).toFixed(2) + '%', cls: v => v === null ? '' : v >= 0.005 ? 'pos' : '' },
    { key: 'heat_6h', label: '熱度 6h', get: r => r.heat_6h, fmt: v => v === null ? '—' : v.toFixed(2) + '×', cls: v => v === null ? '' : v >= 1 ? 'pos' : v < 0.5 ? 'neg' : '' },
    ...
    {compact ? <div className="tablewrap"><table className={'grid compact' + (tight ? ' tight' : '')}>
      ...
      <tbody>{shown.map(r => { const sm = simOf(r, D, R); return <tr key={r.pool_id} className={r.excluded ? 'excluded' : ''}>
        ...
        <td className="l"><Link to={`/pool/${r.pool_id}`}><PoolName symbol={r.symbol} fee_ppm={r.fee_ppm} fee_ppm_observed={r.fee_ppm_observed} hooks={r.hooks} hook_kind={r.hook_kind} hook_flags={r.hook_flags} protocol={r.protocol} /></Link>
          <span className="sub">TVL <b>{fmtUsd(r.tvl_usd)}</b> · 7日量 <b>{fmtUsd(r.vol7_avg_usd)}</b> · CV {fmtNum(r.vol7_cv, 2)} · 熱度 <b className={r.heat_6h === null ? '' : r.heat_6h >= 1 ? 'pos' : r.heat_6h < 0.5 ? 'neg' : ''}>{r.heat_6h === null ? '—' : r.heat_6h.toFixed(2) + '×'}</b> · 週末費/TVL <b className={r.weekend_fee_tvl !== null && r.weekend_fee_tvl >= 0.005 ? 'pos' : ''}>{r.weekend_fee_tvl === null ? '—' : (r.weekend_fee_tvl * 100).toFixed(2) + '%'}</b> · 交易者 <b>{r.trader_count ?? '—'}</b> · top1 {fmtPct(r.top1_share)} · 偏離 <span className={r.price_dev_pct !== null && Math.abs(r.price_dev_pct) > 0.03 ? 'warn' : ''}>{fmtPct(r.price_dev_pct, 1)}</span> · 原始 {fmtPct(r.raw_apr)} <FlagChips flags={r.flags} max={3} /></span>
        </td>
