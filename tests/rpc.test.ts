...
import { isTooManyLogs } from '../scanner/sources/rpc.js'
it('isTooManyLogs 辨識 public RPC 的兩種超量訊息', () => {
  expect(isTooManyLogs(new Error('logs matched by query exceeds limit of 10000'))).toBe(true)
  expect(isTooManyLogs(new Error('Missing or invalid parameters.'))).toBe(true)
  expect(isTooManyLogs(new Error('Too Many Requests'))).toBe(false)
})
