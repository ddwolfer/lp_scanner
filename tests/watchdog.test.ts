...
describe('watchdog assess', () => {
  ...
  it('今天沒啟動 → 警報', () => expect(assess({ ...base, runsToday: [], snapshotsToday: 0 }, now, date)).toMatch(/沒有任何掃描啟動/))
  ...
  it('跑超過 3 小時沒結束 → 卡住警報', () => expect(assess({ ...base, runsToday: [run({ started_at: '2026-09-11T01:00:00Z', finished_at: null, ok: null })] }, now, date)).toMatch(/卡住/))
  it('失敗但已通知 → 不重複', () => expect(assess({ ...base, runsToday: [run({ ok: 0, alert_sent: 1, error: 'x' })] }, now, date)).toBeNull())
  it('失敗且沒通知到 → 補報', () => expect(assess({ ...base, runsToday: [run({ ok: 0, alert_sent: 0, error: 'RPC 429\nstack' })] }, now, date)).toMatch(/RPC 429/))
  it('降級且沒通知到 → 補報', () => expect(assess({ ...base, runsToday: [run({ degraded: 1, alert_sent: 0 })], swapFailedToday: 961 }, now, date)).toMatch(/961/))
  it('成功但沒有快照列 → 產物缺失', () => expect(assess({ ...base, runsToday: [run({})], snapshotsToday: 0 }, now, date)).toMatch(/產物缺失/))
  ...
  it('DB 打不開 → 無法確認也要報', () => expect(assess({ ...base, runsToday: [], dbError: 'SQLITE_CANTOPEN' }, now, date)).toMatch(/無法確認/))
})
