...
| D47 | 發現階段失敗不中斷 | 9/7–9/9 連續三天 07:30 的掃描在池子發現的 getLogs 被公用 RPC 回 429，8 次退避（約 2 分鐘）用完後整個程序中止，三天沒有快照；同一請求在下午手動打 0.7 秒就回，屬於該時段的限流。改法：退避改 12 次、上限 60 秒（合計約 8 分鐘）；發現階段包在 try/catch，失敗只記 log、`last_discovery_block` 不前進，其餘階段照跑，隔天再補掃。 |
