...
| D62 | 損益曲線改對「當時的投入」算 | 使用者發現 MSTR 加倉後曲線在 9/11 前全變 −$600、縱軸被撐大、昨天 +$100 看不見。原因：history 用「目前」deposit_usd 減所有歷史點。改法：(1) 投入時間軸 `capitalAt(t)` = deposit_usd − 之後的 adjust 日誌 `cash_added_usd`（帶正負號的外部本金，再投入的費不算）；(2) 實際淨 = 現值 + 未領費 + 領出到錢包的費 − 當時投入；再投入的費已在現值裡不重加，且在存回的時間點才扣；(3) 模擬改 `simulateWithCapital`：同一部位、固定區間，在事件時點以當時價格加減流動性（減倉最多提到部位市值），再投入事件加流動性並從累計費移到部位；(4) 損益拆解表兩欄同口徑：手續費 = 未領 + 領出 + 再投入（毛額），價格損益 = 現值 − 當時投入 − 再投入；(5) 圖上加倉日畫參考線。效果：MSTR 9/4–9/10 對 $645、9/11 起對 $1,430，曲線連續，9/19 的 +$115 清楚可見；SPY 領出的 $27.62 加回後淨 +$12。Codex review 四輪：明寫再投入多於觀察到的領取時不能變成負領出、est 摘要要跟曲線同來源、再投入用存回時間、模擬與實際的費要同為毛額、減倉不能超過部位市值、多筆明寫各自帶時間戳、再投入時間不早於領取。已知限制（接受）：同一筆領取被拆成兩筆不同時間的再投入時，只記最後一個時間戳；實務上一次加倉一筆，用日誌明寫即可。UI（使用者提議）：手續費合計下加三個子列（未領 / 已領出 / 已再投入，只有實際欄，三列加總等於合計），表下一行投入時間軸（開倉 → 每次加減倉 → 目前），所有卡片格式一致。 |
| D61 | 出區間保本線（含成本、持倉即時） | 借 blockbloomer 的 LP 保本計算器（驗算：其範例 $5000/±2%/APR 5000% 要補 108.6、每小時 28.42，我們的集中流動性公式得 108.5 / 28.42）。改三處：(1) `economics.exitBreakeven`：要補 = 帳面損失（D − 下緣時市值）+ 進場磨損（只對實際買進的幣側）+ 出場磨損（下緣全部換回、上緣為零）+ gas − 已賺的費；天數 = 要補 ÷ 費速；`lifecycleCost` 不再自帶 gas 預設。(2) `pnpm range` 印分項；config gas 每筆 0.6 → 0.2（實測四次開倉 0.05–0.19）。(3) 頭寸頁「保本線」：L 用鏈上真實流動性（每次同步存進 notes，變動時記 `liquidity_changes`）；已賺的費 = 目前未領 + 領過的，領取由快照「未領費掉超過一半」自動偵測（美元計價的未領費跌價也會變小，故設門檻），同一區間若日誌 collect 有精確金額則用日誌；費速用近 7 天已賺差額（不足退回持有期平均並標示）；加倉時自動結算又存回的費視為再投入、不重扣（adjust 日誌 `reinvested_usd` 明寫優先，否則以加倉觀察日與前一日的領取推測，減倉日不算）；出場費率用最近觀察到的交易者總費（含協議費）；已發生成本取 open/adjust/collect 日誌的 gas_usd + costs_usd；`position_snapshots` 加 `taken_at` 讓領取比對用精確時間，舊快照退回台北日期。Codex review 十輪：修了進場只算幣側、上緣不換幣、費速口徑、跌價誤判領取、再投入重扣、減倉日、時間戳比對、動態池費率等；剩下「舊資料沒有觀察紀錄」以日誌明寫解決。上線時三筆頭寸：TSLA 還差 $52、27.9 天；MSTR 還差 $157、50 天；SPY 還差 $5、6.6 天。 |
| D60 | 手續費改算 LP 實得（扣協議費） | 解讀 brucexu 那篇惡意 hook 的推文時，解碼自己的交易發現 Uniswap 協議費已在 Robinhood Chain 啟用：v4 MSTR 池設定 LP 費 2500ppm，slot0.protocolFee=400，Swap 事件的 fee 欄是 2899（交易者付的總費，`swapFee = pf + lpFee − pf×lpFee/1e6`）；協議費先從輸入扣、LP 費對餘額收，**LP 實得 ppm = 事件費 − pf**（2899−400=2499=2500×(1−0.0004)）。v3 則是 slot0.feeProtocol=0x66，兩方向都抽 LP 費的 1/6。我們原本直接用事件費當 LP 收入，v4 高估約 16%、v3 約 20%。修正：新增 `scanner/metrics/protocolFee.ts`（純函式，解碼兩個方向 + `lpFeePpm`）；`aggregateHourly` 改用 LP 實得費率，且費基改成輸入側（USDG 是輸出時除以 (1−總費) 還原，不用池價換算以免大單偏差）；`pools.pf_ppm0/pf_ppm1/pf_block` 快取 7 天，靜態 v4 池由事件反推、不必上鏈，只有 v3 與動態費率 v4 需讀 slot0；讀不到就標 `protocol_fee_unknown` 且 `pool_snapshots.fee_basis` 記 `gross`（不靜默當成 0）。`fee_ppm_observed` 維持交易者總費（費率區間排除與顯示用）。回放腳本讀窗口起點的歷史協議費，並比對窗口終點，變動就警告。效果：MSTR 一日回放的估計費對 feeGrowth 精確值從高估 9–16% 降到 4.5%。**9/16 以前的 `fees_24h_usd` 是交易者總費，偏高 14–20%，未回填**。Codex review 五輪，採納：驗證事件原始值（2899 非 2900）、兩個方向分開存、快取要有時效、讀取失敗不可當 0、回放要用當期設定、hourlyRows 與 regime 報表輪也要套用。 |
| D59 | 重試改看狀態碼（9/15） | 9/15 掃描 155/1295 個池 `swap_fetch_failed`（含使用者持倉的 TSLA 池，當天成交量記成 0），但 `scan_runs.ok=1`、失敗比例 12% 未達 20% 門檻，只在摘要末尾帶一句。查因：失敗一秒一個、沒有經過退避，代表錯誤沒被判定成可重試；log 只印 `e.message`（「HTTP request failed.」）看不到原因。D57 的字串比對只涵蓋 502/503，Cloudflare 的 520–524 與不帶狀態的 `HttpRequestError` 都漏掉。改法：`isRetryable()` 以狀態碼為準（429 與任何 5xx 重試，其餘 4xx 視為永久錯誤），沿 `cause` 鏈走訪（viem 會把 HttpRequestError 包在 ContractFunctionExecutionError 底下），沒有狀態碼才看錯誤名稱與字串，並移除鬆散的 `5\d\d` 比對（errText 含請求內文會誤中區塊號）；swap 失敗的 log 改印 `errText`。11 條測試。Codex review 三輪：永久 4xx 不該因錯誤名稱重試、數字比對誤判、cause 鏈未走訪，全部修正。 |
| D58 | 掃描異常的 Telegram 通知與獨立巡檢 | 使用者要求「腳本有問題也要 TG 通知」。三層：(1) 掃描內：swap 抓取失敗比例 > 20%、一個都沒抓、或發現階段失敗 → 摘要開頭 ⚠️ 含數字與補跑指令，`scan_runs.degraded=1`；程式中止 → catch 先送 ❌ 再 rethrow，`alert_sent` 記通知是否送達；`--sim-only` 記 NULL 不覆蓋。(2) 獨立 launchd `com.lp-scanner.watchdog` 10:30 與 13:30（台北）跑 `pnpm watchdog`：看產物不看 exit code（scan_runs 當天最後一次 + pool_snapshots 當天列數），成功靜默，問題才送 🩺 含缺什麼 / 怎麼補 / 最可能原因；跑超過 3 小時未結束視為卡住；失敗或降級但通知沒送達 → 補報；資料庫打不開也報「無法確認」。判斷邏輯在 `scanner/watchdog.ts` 純函式，10 條測試。同一天同一組問題只送一次（`meta` 鍵 `watchdog_alert:<日期>:<種類>`，只記送達的），13:30 仍會檢查、狀態變好自然靜默（agent_flow_studio 提醒）。設計參考 agent_flow_studio session 的巡檢經驗（監視者不與被監視者共死、排程只有一個真相來源、未知時寧可通知）與 Codex plan review 五點。 |
| D57 | 429 沒被重試（9/11） | 9/11 07:30 的掃描 961 / 1258 個池 `swap_fetch_failed`（含 SPY、GLD、SGOV 主池），當天排名與 Telegram 不可信。原因：viem 的 HttpRequestError 把「Too Many Requests」放在 `details`，`message` 只有「RPC Request failed.」，重試的正則只看 message，429 一次都沒重試。修法：判斷字串改成 message + details + shortMessage + cause.message + status。另實測公用 RPC 現在 getLogs 約 2 秒一次才穩（8 次 1 次 429），5 秒一次全過；Alchemy 免費層 getLogs 只給 10 個區塊，不能當備援。當天用 `RPC_CONCURRENCY=2 RPC_GAP_MS=750` 手動重跑。Codex review 補兩點：`isTooManyLogs` 也只看 message（exceeds limit 同樣在 details）、丟出純字串時會變空字串 → 抽成 `errText()` 共用。 |
| D56 | 三項驗證：自稀釋、留一法、各池自身 regime | (1) 策略報表改印估計費 / feeGrowth 費（逐段，整段未出區間用 global、出過用 inside、不合格用估計並標 *）/ 兩種淨 / 份額中位、p95、最大。AMD ±10% 靜態 34 天：feeGrowth $287、淨 +$280；份額中位 2.6%、p95 3.9%、最大 6.5%，自稀釋校正約 −3%；±6% 最大 10%。SPY/QQQ ±0.85% 靜態份額 0.3% / 最大 2.4%，但出過區間拿不到 feeGrowth；±1.5% 精確 $12.97（估 1.14 倍）。(2) 留一法（每週重置 $1,000）：AMD 政策 ±10% 五週 37.4 / 12.2 / 31.4 / 113.3 / 13.6，合計 $208、中位 $31、最差 $12、最好週佔 54%、留一法最低 $95；SPY/QQQ ±0.85% 為 −0.1 / −0.4 / 3.3 / 8.7 / 0.6，合計 $12、最好週佔 72%、留一法最低 $3.4。(3) 各池自身分類：AMD 前四週自身震盪、第五週自身上漲趨勢（+11.8%），那週淨仍 +$13.6；SPY/QQQ 五週四週震盪、第五週自身下跌趨勢。**關鍵發現**：AMD $1,000 的份額從 8 月初的 2.4% 掉到 9 月的 0.14%，池的流動性在五週內成長約 17 倍，8 月的高 alpha 是池子還薄時的產物；往前看應以第五週（份額 0.13%、週費 $44、淨 +$13.6）為基準，不是 34 天的 +$280。 |
| D55 | 策略模擬與費用來源校準 | 回應 GPT：(1) `pnpm policies`：整段 34 天一個頭寸，四種管理策略（不重開 / 出區間 24h 重開 / 超界 0.25% 重開 / 每週重開），成本 = 重平衡量 × (池費率 + 0.1% 滑價) + gas $1.5。結果：兩個池都是「不重開」最好，任何重開只增加成本；AMD ±10% 靜態 34 天 feeGrowth 費 $255、IL −$8、淨 +$247（97%）；每週重開扣成本後 $208。SPY/QQQ ±0.85% 靜態估計 $24（無法精確驗證），每週重開只剩 $6.7。(2) 新增 feeGrowthGlobal 精確路徑：價格整段沒離開區間時 Δglobal = Δinside，不依賴 tick；SPY/QQQ ±1.5σ / ±2σ 五週中四週因此有精確值，估/精確逐週為 1.05、—、0.82、1.18、1.82，偏差不是固定 2 倍，窄區間與價格貼邊時偏高。(3) SPY/QQQ 政策 ±0.85% 五週全部只有估計值：費 $19.97、IL −$6.96；敏感度 1x +$13.0、0.75x +$8.0、0.5x +$3.0，同週校準 +$10.7。(4) 波動正規化 ±1.5σ 五週淨：SPY/QQQ $7.6（費加權留存 67%）、AMD $256（84%）；同樣的區間覆蓋下 AMD 留存率仍較高，「pair 相關性帶來結構優勢」在這五週沒有出現，因為 SPY/QQQ 比值相對自身 σ 的漂移跟 AMD 一樣大，而費密度低得多。(5) 週窗口的 $199 正名為「五個每週重置的 $1,000 窗口 alpha 合計」；被動持有一個頭寸的數字是 (1) 的 +$247。附錄 `docs/policy-backtest-2026-09-10.txt`。 |
| D54 | 多 regime 回測（`pnpm regimes`） | 依 GPT 建議用規則切窗避免挑樣本：從最新區塊往回切 7 天窗口，以 SPY/QQQ 池存在為準（7/13 起，5 個窗口）；分類用 SPY/QQQ 週報酬 ≥1% 為趨勢、否則依週波動中位數分高/低波動震盪；每池跑政策區間（SPY/QQQ ±0.85%、股票/USDG ±10%）與前一週 σ 的 ±1/1.5/2σ；輸出 feeGrowth 費、估計費、LP−HODL、淨、留存率、LP−50/50。核心抽成 `scripts/lib/replay-core.ts`，swap 快取在 `.cache/replay`。結果（$1,000、政策區間、五週淨）：AMD/USDG 1% +$37 / +$12 / +$31 / +$113 / +$6 = +$199，留存率 100/76/99/97/12%；SPY/QQQ +$0.3 / −$0.1 / +$3.2 / +$8.6 / −$0.6 = +$11.5，留存率 11/−2/99/87/−31%，窄區間五週出區間 9 次；MSTR 只有兩週（第一週池齡兩天、只有估計值，捨棄），第二週 +$45、82%。結論：五天窗口得到的「SPY/QQQ 留存率有結構優勢」不成立，它的留存率在不同週之間從 −31% 到 99% 擺盪，而 AMD/USDG 在 AMD 橫盤的四週留存率 76–100%、只在 AMD 單週 +13.6% 時掉到 12%。留存率是 regime 的函數，不是池的屬性。SPY/QQQ 多數窗口 feeGrowth 無效（窄區間邊界 tick 常被清空），只能用已知高估約 2 倍的估計值，真實數字更差。附錄 `docs/regime-backtest-2026-09-10.txt`。 |
| D53 | replay 第三輪修正 | (1) 區間改在「顯示價」上定義（USDG 是 token0 時顯示價 = 1/P），再換回 tick；AMD、MSTR 的池 USDG 都是 token0，前一輪的 ±10% 實際是股價 −9.1%/+11.1%，修後 MSTR ±10% 從 [130.93–160.32] 變 [129.63–158.72]，數字幾乎不變但定義正確；印出 token0/token1。(2) 加 `--lower/--upper`（顯示價），可直接重放真實頭寸的不對稱區間。(3) 外推邊界 tick 前先確認價格整段都在原始區間內，否則判無效。(4) 拿掉用估計 totalFeeY 當 feeGrowth 上限的守門。(5) 小時估計的手續費改按輸入側（v4 Swap 事件 amount 為使用者視角，負 = 輸入；實測 300 筆整數 USDG 全在負側）。固定窗口結果：MSTR ±10% 淨 +$0.78、AMD ±10% +$0.77、SPY/QQQ 截圖區間 [1.0613–1.0795] 費 $2.42、IL −$0.79、淨 +$1.63、留存 67%。下一步依 GPT 建議做多 regime 回測（`--from/--to` 已支援，鏈上 swap 可回溯到 7 月）。 |
| D52 | `pnpm replay` 通用化與固定窗口 | 回應 GPT 的 code review：(1) 淨（LP−HODL）改用 feeGrowth 費（有效時），並加「留存率 = 淨 / 費」；(2) 腳本通用化到任意 v4 池：X = token0、Y = token1、decimals 從 tokens 表取，L_raw = L × 10^((d0+d1)/2)，D 以 Y 計、用窗口起點的 Y 美元價換算、結果用終點價換回，`pnpm replay SPY/QQQ --fee=0.05` 直接用同一條路徑，`spy-qqq` 腳本退役；(3) `--from/--to` 固定區塊，比較用 55203441–59353441（9/5 14Z → 9/10 11Z）；(4) feeGrowth 對照的有效條件改為「邊界 tick 在頭尾兩個區塊都有 liquidityGross」，因為 tick 在窗口內被清空或新建都會重設 feeGrowthOutside（MSTR ±10% 原本印 $40.85，真值 $22.21，就是上緣 tick 在終點被清掉）；不合格就往外找最近合格的 tick 當對照。固定窗口結果（$1,000）：MSTR ±10% 費 $22.21、IL −$21.43、淨 +$0.78、留存 4%；AMD ±10% 費 $17.75、IL −$17.05、淨 +$0.71、留存 4%；SPY/QQQ ±0.85% 費 $2.49、IL −$0.81、淨 +$1.68、留存 67%。估/精確：USDG 池 102–103%，SPY/QQQ 204%。名稱用「歷史 feeGrowth 費」，不叫 counterfactual。 |
| D51 | 用 feeGrowthInside 校準模擬器 | GPT 指出 Swap 事件的 `liquidity` 是 swap 後的 active liquidity，跨 tick 的大單會讓「成交量 × 費率 × 份額」有系統誤差。加精確對照：Alchemy 支援歷史 eth_call，`StateView.getFeeGrowthInside(pool, tl, tu)` 在窗口頭尾的差 × L / 2^128 就是該區間每單位流動性的真實手續費（用真實 MSTR 頭寸驗證：算出 $11.17，dashboard 實收 $10.96）。同一 4.9 天窗口，估計 / 精確：AMD ±10% 98%、±25% 98%；MSTR ±10% 109%、±25% 109%；**SPY/QQQ 各區間都是 204%**。USDG 池流動性分布寬，事件的 post-swap liquidity 接近真實；SPY/QQQ 全部 LP 擠在現價 ±1%，套利大單穿過厚的區段後停在薄的位置，用終點流動性算份額高估一倍。逐筆用前一筆的流動性並沒有更準（106–120%）。限制：假想區間的邊界 tick 若未初始化又被穿越，feeGrowthInside 無意義（MSTR ±5% 案例），腳本會標「無效」。修正後同窗口 $1,000 淨（LP−HODL）：MSTR ±10% +$2.36、SPY/QQQ 截圖區間 +$1.69、AMD ±10% +$0.25，三者差距在幾美元內，不宣告排名。結論：`pnpm replay` 對 USDG 池可信（±10%），對流動性極集中的池必須看精確費欄位。 |
| D50 | `pnpm replay`：LP−HODL 才是 KPI | GPT 指出拿 SPY/QQQ 的「淨損益」比 AMD/MSTR 的「手續費」不是同一個指標。加 `pnpm replay <pool|SYMBOL> [days] [D] [ranges]`：抓最近 N 天真實 swap，對每個區間印手續費、LP−HODL（不含費）、淨（費 + LP−HODL）。同一 4.9 天窗口（9/5 13Z → 9/10 09Z）$1,000：AMD ±10% 費 $20.6、LP−HODL −$21.0、淨 −$0.3（AMD 單邊 +9.3%）；MSTR ±10% 費 $24.4、LP−HODL −$20.9、淨 +$3.6（MSTR −8.8%）；SPY/QQQ 截圖區間費 $5.3、LP−HODL −$0.9、淨 +$4.4。趨勢週裡高費池的費幾乎剛好被 IL 吃掉，SPY/QQQ 費薄但留得住。模擬驗證：MSTR 實際費 $10.96 vs 估 $9.17（73h），SPY 每小時估值比實際高約 25%。手續費算法是 `成交量 × LP 費率 × 每小時份額（Swap 事件的 liquidity）`，不是 feeGrowth；v4 協議費（0.05% 池為 1.25 bps）從輸入先扣再算 LP 費，對 LP 實收影響約 0.01%，未納入。 |
| D49 | SPY/QQQ 池實測 | 使用者拿 GPT 對 SPY/QQQ 0.05% 池的評估來問。用 `pnpm spy-qqq <SPY$> <天>` 抓真實 swap（4.9 天 38,779 筆）跑 §7 模擬：池總費 $809/日、費/TVL 0.048%/日；$1,000 放 ±0.85% 窄區間手續費 $1.08/日（費 APR 39%），±1.4% 為 $0.66/日，±3.3% 為 $0.28/日。同期比值從 1.0735 單邊滑到 1.0656（QQQ 跑贏），窄區間淨損益約 0，寬區間為負。結論：這是股票對股票池裡唯一 IL 真的小的組合（兩邊 σ 都 ≤1%），但費太薄，只有極窄區間加勤調倉才勉強打平持有；不納入掃描宇宙，需要時用此腳本查。 |
| D48 | 股票 × SPY 池不納入 | 使用者看到 SPY/TSM 池 TVL 比 TSM/USDG 高，研究「股票對股票」池是否值得掃（`pnpm spy-pairs`：鏈上找出 53 個無 hook 固定費率的 SPY×股票 v4 池，DexScreener 取 TVL/量，用 `pool_hourly` 算股票對 SPY 比值的週波動）。結果：(1) 費率幾乎全是 0.05%，費/TVL 日 0.04–0.18%，同股票的 USDG 主池是它的 3–10 倍（META 0.30% vs 2.22%、AMD 0.09% vs 0.70%）；(2) 「相關性降低 IL」不成立，SPY 週波動只有 0.8%，比值的 σ₇ 與股票對美元的 σ₇ 幾乎相等（AMD 5.3% vs 5.3%、PLTR 9.4% vs 9.4%）；(3) 這些池的量來自路由中繼，LP 收的是路由費，不是交易者的費。結論：不納入宇宙，C1 維持。 |
| D47 | 掃描卡死與 RPC 限流的修正 | 9/7–9/9 連續三天 07:30 的掃描在池子發現的 getLogs 被公用 RPC 回 429，8 次退避用完後整個程序中止，三天沒有快照。補跑時又在第 160 個池（META，單日 3.8 萬筆 swap）停住 30 分鐘：「超過 10k logs」的錯誤訊息（`Missing or invalid parameters`）同時符合退避的正則，被當成限流重試到上限才交給對半切，每一層切分前空等數分鐘。修法：(1) `isTooManyLogs` 的錯誤直接丟出、不退避，META 一天的 swap 34 秒抓完；(2) 退避改 12 次、上限 60 秒；(3) 發現階段包 try/catch，失敗只記 log、`last_discovery_block` 不前進，其餘階段照跑；(4) `RPC_CONCURRENCY`、`RPC_GAP_MS`、`RPC_DEBUG` 環境變數可調速與印每次錯誤。其他公開節點（publicnode 拒絕 getLogs、arrowrpc / ordofi 掛掉）都不能當備援。 |
...
| D42 | 真實成本計算（`pnpm costs`） | gas = receipt 的 `gasUsed × effectiveGasPrice`，ETH/USD 用 DexScreener WETH/USDG 主池價；交易 hash 由 Alchemy `getAssetTransfers`（erc20 + external，from/to 各一次）取得，只算自己發起的交易。代幣流向從 receipt 的 ERC20 `Transfer` log 讀 USDG 與白名單股票。期間股票代幣淨變動為 0 時，`USDG 淨變動 − gas` = 實際淨利（swap 費與滑價已含在成交價）。SPCX 實驗實測：6 筆 gas $2.15、淨利 +$5.25。操作流程寫在 `.claude/skills/lp-ops/SKILL.md`。 |
