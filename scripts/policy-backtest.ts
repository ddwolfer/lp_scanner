...
import { makeCtx, resolvePool, loadSwaps, tsOf, hourlyRows, simulateStateful, type Policy } from './lib/replay-core.js'
...
for (const [target, fee, widths] of specs) {
  ...
  console.log('半寬    策略            重開  在區間   手續費    成本   LP 終值   HODL 終值    淨(LP−HODL−成本)')
  for (const w of widths) for (const [name, pol] of policies) {
    const r = simulateStateful(pool, hs, w, pol, D_USD, yUsd, GAS, pool.feePpm / 1e6 + 0.001)
    console.log(`±${String(w).padEnd(5)} ${name.padEnd(14)} ${String(r.recenters).padStart(3)}   ${(r.inRange * 100).toFixed(0).padStart(4)}%  ${f(r.fees)}  ${f(r.costs)}  ${f(r.lpEnd)}  ${f(r.hodlEnd)}   ${f(r.net)}`)
  }
}
