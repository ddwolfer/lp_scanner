...
import { makeCtx, resolvePool, loadSwaps, tsOf, replayWindow, type Case, type WindowResult, type PoolInfo, loadProtocolFeeAt } from './lib/replay-core.js'
...
for (const w of usable) {
  await loadProtocolFeeAt(ctx, spyqqq.pool, w.from)   // D60
...
for (let i = 0; i < usable.length; i++) {
  ...
  for (let k = 0; k < pools.length; k++) {
    ...
    await loadProtocolFeeAt(ctx, p.pool, w.from, w.to)   // D60：報表這一輪也要用該窗口的協議費
