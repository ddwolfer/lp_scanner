...
export interface SnapshotRow {
  ...
  age_days: number | null; vol7_avg_usd: number; vol7_cv: number | null; raw_apr: number | null; flags: string[]; excluded: number; fee_basis?: string
}
export function writeSnapshot(db: Database.Database, r: SnapshotRow) {
  db.prepare(`INSERT OR REPLACE INTO pool_snapshots(pool_id,date,is_weekday,tvl_usd,volume_24h_usd,fees_24h_usd,price_usd,price_ref_usd,price_dev_pct,swap_count,fee_ppm_observed,vol_6h_usd,vol_1h_usd,age_days,vol7_avg_usd,vol7_cv,raw_apr,flags,excluded,fee_basis)
    VALUES (@pool_id,@date,@is_weekday,@tvl_usd,@volume_24h_usd,@fees_24h_usd,@price_usd,@price_ref_usd,@price_dev_pct,@swap_count,@fee_ppm_observed,@vol_6h_usd,@vol_1h_usd,@age_days,@vol7_avg_usd,@vol7_cv,@raw_apr,@flags,@excluded,@fee_basis)`)
    .run({ fee_ppm_observed: null, vol_6h_usd: null, vol_1h_usd: null, fee_basis: 'lp_net' as string, ...r, flags: JSON.stringify(r.flags) })   // D60：新列的費是 LP 實得
}
